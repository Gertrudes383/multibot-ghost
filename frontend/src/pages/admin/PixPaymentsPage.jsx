...
import { getPixPayments } from '@services/admin.service'
...
export default function PixPaymentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pix-payments', status, page],
    queryFn: () => getPixPayments({ status, page, limit: 20 }),
  ...
  return (
    <div className="space-y-6 p-6">
      <h1 className="text-2xl font-bold text-[var(--mb-text-primary)]">Pagamentos PIX</h1>
      ...
      <Surface>
        {isLoading ? (
          ...
          <EmptyState title="Sem pagamentos" description="Nenhum pagamento PIX encontrado" />
        ) : (
          <>
